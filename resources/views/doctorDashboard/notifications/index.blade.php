                    ...
                    <div dir="rtl"
                        ...
                        <div class="shrink-0 flex gap-2">
                            @if (is_null($notification->read_at))
                                <form action="{{ route('doctor.notifications.mark-as-read', $notification->id) }}"
                                    method="POST">
                                    @csrf
                                    <button type="submit" class="px-3 py-1.5 text-sm font-medium text-primary-600 border border-primary-600 rounded-md hover:bg-primary-50 dark:text-primary-400 dark:border-primary-400 dark:hover:bg-primary-900/50">
                                        Mark as Read
                                    </button>
                                </form>
                            @else
